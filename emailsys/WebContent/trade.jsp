...
<div class="page-header">
		...
		<select value="功能查询" >
			<option>精准查询</option>
			<option>日期查询</option>
			<option>同交同策查询</option>
			<option>同品种盈利查询</option>
		</select>
...
<script type="text/javascript">
	...
	$.ajax({
		...
		url: "showPager_change.action"
			...
			$('#paper').empty();
			for (var i = 1; i <= res.paperCount; i++) {
			$('#paper').append('<option value = "' + i + '"> ' + i + '/' + res.paperCount + '</option>');
			} 
	})
		//给select一个onchange事件
		//读取admin是3的人
		...
		$("#de").click(function(){
			...
			/* $.ajax({
				...
				data: {"pageNum":$("#paper").val()}
					...
					$('#paper').append('<option value = "' + i + '"> ' + i + '/' + res.recordCount + '</option>');
					} 
			}) */
				}
			})	
